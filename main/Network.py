from keras.layers import Input, Conv2D, Conv2DTranspose, Concatenate, GlobalAveragePooling2D, Reshape, Multiply
...
def Attention_block(input, channels, r):
    # Squeeze
    se = GlobalAveragePooling2D()(input)
    # Excitation
    se = Reshape((1,1,channels))(se)
    se = Conv2D(channels, kernel_size=1, activation='relu', padding='same')(se)
    se = Conv2D(channels, kernel_size=1, activation='sigmoid', padding='same')(se)
...
def build_mbllen(input_shape):
    ...
    def EM(input, kernal_size, channel):
        ...
        # Add SE block
        conv_6 = Attention_block(conv_6, channel, r=16)
